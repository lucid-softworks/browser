...
fn is_field_vchar_or_obs_fold(b: &u8) -> bool {
    match b {
        b' ' | b'\t' => true,
        0x21..=0x7E => true,
        // obs-text (RFC 7230 §3.2.6: `obs-text = %x80-FF`) is a legal header-value byte. Upstream
        // ureq omits this range, which silently drops e.g. a Set-Cookie carrying UTF-8 (a BOM) — so
        // include it. (Local patch via [patch.crates-io].)
        0x80..=0xFF => true,
        _ => false,
    }
}
